...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, FlatList, ActivityIndicator, Modal, Pressable, Dimensions, ScrollView } from 'react-native';
...
export default function Conversation() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      async (snap) => {
        ...
        // Clear pending message if a matching real message has arrived
        setPendingMessage((pm) => {
          if (!pm || !pm._local) return pm;
          // Check if any real message matches the pending one (same text and senderId)
          const matchingMessage = items.find(m => 
            m.senderId === pm.senderId && 
            m.text === pm.text &&
            // Message was created within the last 5 seconds (to avoid matching old messages)
            (() => {
              const msgDate = m.createdAt?.toDate ? m.createdAt.toDate() : (m.createdAt ? new Date(m.createdAt) : null);
              const pendingDate = pm.createdAt instanceof Date ? pm.createdAt : new Date(pm.createdAt);
              if (!msgDate) return false;
              return Math.abs(msgDate.getTime() - pendingDate.getTime()) < 5000;
            })()
          );
          return matchingMessage ? null : pm;
        });
  ...
  const sendMessage = async () => {
    ...
    // If online, send immediately (don't show pending message - real one will arrive quickly)
    try {
      setSending(true);
      setInput('');
      ...
      // Don't set pendingMessage for online sends - Firestore listener will update immediately
    } catch (e) {
      console.error('Error sending message:', e);
      // On error, we could show an error indicator, but for now just log it
  ...
  // Deduplicate messages: exclude pending/queued messages that match real messages
  const displayMessages = useMemo(() => {
    // Combine all messages
    const allMessages = [...(pendingMessage ? [pendingMessage] : []), ...queuedMessages, ...messages];
    
    // Deduplicate: if a pending/queued message matches a real message, exclude the pending/queued one
    const deduplicated = [];
    const seenRealIds = new Set();
    const seenContentKeys = new Set();
    
    // First pass: add all real messages
    for (const msg of allMessages) {
      if (!msg._local && !msg.queuedAt) {
        seenRealIds.add(msg.id);
        deduplicated.push(msg);
      }
    }
    
    // Second pass: add pending/queued messages only if no matching real message exists
    for (const msg of allMessages) {
      if (msg._local || msg.queuedAt) {
        // Check if a real message with same content exists (same sender, same text, within 10 seconds)
        const hasMatchingRealMessage = messages.some(m => {
          if (m._local || m.queuedAt) return false;
          if (m.senderId !== msg.senderId || m.text !== msg.text) return false;
          
          // Check timestamp proximity (within 10 seconds) to avoid matching old messages
          try {
            const msgDate = msg.createdAt instanceof Date ? msg.createdAt : (msg.createdAt?.toDate ? msg.createdAt.toDate() : new Date(msg.createdAt || Date.now()));
            const realDate = m.createdAt?.toDate ? m.createdAt.toDate() : (m.createdAt ? new Date(m.createdAt) : null);
            if (!realDate) return false;
            return Math.abs(realDate.getTime() - msgDate.getTime()) < 10000;
          } catch {
            return true; // If date parsing fails, assume match
          }
        });
        
        // Only add if no matching real message exists
        if (!hasMatchingRealMessage) {
          const contentKey = `${msg.senderId}-${msg.text}`;
    ...
    // Add intro message at the end (appears at top when inverted) if there are real messages
    const hasRealMessages = messages.some(m => !m._local && !m.queuedAt);
    if (hasRealMessages && deduplicated.length > 0) {
      deduplicated.push({
        id: '__intro_message__',
        _intro: true,
    ...
    return deduplicated;
  }, [pendingMessage, queuedMessages, messages, displayName]);

  // Find the latest own message from displayMessages (includes pending/queued)
  const latestOwnMessageId = useMemo(() => {
    const first = displayMessages.find(m => m.senderId === user?.uid);
    return first ? first.id : null;
  }, [displayMessages, user?.uid]);

  const renderMessage = ({ item, index }) => {
    // Render intro message (avatar, name, tip)
    if (item._intro) {
      return (
        <View style={styles.introMessageContainer}>
          <View style={styles.emptyAvatar}>
            <Text style={styles.emptyInitial}>{String(item.displayName.trim()).split(' ')[0]?.[0]?.toUpperCase() || '?'}</Text>
          </View>
          <View style={{ marginTop: 10 }}>
            <Text style={styles.emptyName}>{String(item.displayName.trim())}</Text>
    ...
    const prev = displayMessages[index + 1]; // messages are in desc order
    ...
    return (
      <View>
        ...
        {(selectedMessageId === item.id || (mine && item.id === latestOwnMessageId)) && (
        ...
        <View style={styles.modalOverlayCenter}>
          <View style={styles.fbModalCard}>
            ...
            >
              ...
              <Text style={styles.fbModalTitle}>Delete Conversation</Text>
              <Text style={styles.fbModalMessage}>This will permanently delete all messages in this conversation.</Text>
            </ScrollView>
        ...
        <View style={styles.modalOverlayCenter}>
          <View style={styles.fbModalCard}>
            ...
            >
              ...
              <Text style={styles.fbModalMessage}>{feedbackText}</Text>
            </ScrollView>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  introMessageContainer: { alignItems: 'center', justifyContent: 'center', paddingTop: 20, paddingBottom: 20 },
