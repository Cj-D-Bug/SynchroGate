...
const ParentProfile = () => {
  ...
  // ✅ Check linked students status - Using real-time listener approach
  useEffect(() => {
    if (!currentParent?.id && !currentParent?.uid) {
      setIsLinked(false);
      setLinkedStudents([]);
      return;
    }
      ...
      if (studentsToFetch.length > 0) {
        Promise.all(
          studentsToFetch.map(async (student) => {
            try {
              const studentDocRef = doc(db, 'users', student.studentId);
          ...
          if (finalStudents.length > 0) {
            setIsLinked(true);
            setLinkedStudents(finalStudents);
          } else {
            setIsLinked(false);
            setLinkedStudents([]);
          }
              ...
              <View style={styles.linkedStudentsContainer}>
                {linkedStudents.map((student, index) => (
                  <View key={student.id || index} style={styles.linkedStudentItem}>
                    ...
                    <Text style={styles.linkedStudentText}>{student.studentName || 'Student'}</Text>
