    ...
    const checkLinkedParents = async () => {
      try {
        ...
        await withNetworkErrorHandling(async () => {
          ...
          // Try with status filter first, then without if no results
          const queries = [];
          if (studentUid) {
            queries.push(query(
          ...
          for (const q of queries) {
            try {
              ...
              linksSnap.docs.forEach(doc => {
                const data = doc.data();
                const parentId = data.parentId || data.parentUid || data.uid || doc.id;
                
                // Skip if we've already seen this parent
                if (seenParentIds.has(parentId)) return;
                seenParentIds.add(parentId);
                
                // Only include if status is active or if status field doesn't exist (for legacy data)
                const status = data.status || '';
                if (status === '' || status === 'active') {
                  // Check if we have parentName in the link document
                  const linkParentName = data.parentName || 
                    (data.parentFirstName && data.parentLastName 
                      ? `${data.parentFirstName} ${data.parentLastName}`.trim() 
                      : '');
                  
                  if (linkParentName) {
                    // We have the name from the link document
                    allResults.push({
                      id: doc.id,
                    ...
                    allResults.push({
                      id: doc.id,
                      parentName: '', // Will be filled after fetching
                      parentId: parentId,
                      relationship: data.relationship || '',
                    });
                  }
              ...
              console.log('Error executing query:', queryError);
              // Continue with other queries
            }
          }
          
          // Also check linked_parents collection as fallback
          if (allResults.length === 0) {
            try {
              const linkedParentsDocRef = doc(db, 'linked_parents', studentUid || studentIdNumber);
              const linkedParentsDoc = await getDoc(linkedParentsDocRef);
              if (linkedParentsDoc.exists()) {
                const data = linkedParentsDoc.data();
                const parents = data.parents || data.items || [];
                if (Array.isArray(parents) && parents.length > 0) {
                  parents.forEach((parent, index) => {
                    if (parent && typeof parent === 'object') {
                      const parentId = parent.parentId || parent.id || parent.uid || `parent_${index}`;
                      if (!seenParentIds.has(parentId)) {
                        seenParentIds.add(parentId);
                        const parentName = parent.parentName || parent.name || (parent.firstName && parent.lastName 
            ...
            } catch (linkedParentsError) {
              console.log('Error checking linked_parents collection:', linkedParentsError);
            }
          }
          
          // Fetch all parent names from users collection that we couldn't get from link documents
          if (parentIdsToFetch.size > 0) {
            const parentNamePromises = Array.from(parentIdsToFetch).map(async (pid) => {
              try {
                ...
                if (parentDoc.exists()) {
                  ...
                  const middleName = parentData.middleName || '';
                  const fullName = [firstName, middleName, lastName].filter(Boolean).join(' ').trim();
                  return { parentId: pid, parentName: fullName || 'Parent' };
                }
              } catch (fetchError) {
                console.log(`Error fetching parent ${pid}:`, fetchError);
              }
              return { parentId: pid, parentName: 'Parent' };
            });
            
            const parentNames = await Promise.all(parentNamePromises);
            const parentNameMap = new Map(parentNames.map(p => [p.parentId, p.parentName]));
            
            // Update allResults with fetched parent names
            allResults.forEach(result => {
              if (!result.parentName && parentNameMap.has(result.parentId)) {
                result.parentName = parentNameMap.get(result.parentId);
              }
            });
          }
          ...
          console.log(`✅ Found ${uniqueParents.length} linked parent(s) for student ${studentUid || studentIdNumber}`);
        ...
        if (error.type === 'no_internet' || error.type === 'timeout' || error.type === 'unstable_connection') {
          ...
          setNetworkErrorVisible(true);
          setTimeout(() => setNetworkErrorVisible(false), 5000);
          // Don't set to empty array on network error - keep previous state
          return;
        }
        // Only set to empty array if it's a real error (not network)
        setLinkedParents([]);
      }
    };
    checkLinkedParents();
