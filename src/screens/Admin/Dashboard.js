  ...
  const fetchAllCounts = async () => {
    try {
      ...
      const { data, fromCache } = await fetchWithCache('admin_dashboard_counts', async () => {
        ...
        // Helper function to check if document ID matches pattern YYYY-XXXXX (e.g., "2022-00488")
        // Excludes Firebase UID-like IDs (e.g., "K3p9LKYTMRcNvN6zs42luWZw4VK2")
        const isValidIdPattern = (docId) => {
          if (!docId || typeof docId !== 'string') return false;
          // Pattern: YYYY-XXXXX (4 digits, hyphen, 5 digits)
          const idPattern = /^\d{4}-\d{5}$/;
          return idPattern.test(docId);
        };

        // Filter students: only count documents with ID pattern YYYY-XXXXX (not Firebase UIDs)
        const studentsWithId = allStudents.filter(s => isValidIdPattern(s.id));
        
        // Filter parents: only count documents with ID pattern YYYY-XXXXX (not Firebase UIDs)
        const parentsWithId = allParents.filter(p => isValidIdPattern(p.id));
        ...
        // Check using document ID (which matches the pattern YYYY-XXXXX)
        const unlinkedStudents = studentsWithId.filter(s => !linkedStudentIds.has(s.id));
        // Check using document ID (which matches the pattern YYYY-XXXXX)
        const unlinkedParents = parentsWithId.filter(p => !linkedParentIds.has(p.id));

        return {
          totalStudents: studentsWithId.length,
          totalParents: parentsWithId.length,
  ...
  const fetchStudentsQrCounts = async () => {
    try {
      ...
      const { data, fromCache } = await fetchWithCache('admin_dashboard_qr_counts', async () => {
        ...
        // Helper function to check if document ID matches pattern YYYY-XXXXX
        const isValidIdPattern = (docId) => {
          if (!docId || typeof docId !== 'string') return false;
          const idPattern = /^\d{4}-\d{5}$/;
          return idPattern.test(docId);
        ...
        usersSnap.docs.forEach(docRef => {
          ...
          // Only count documents with ID pattern YYYY-XXXXX (not Firebase UIDs)
          if (isValidIdPattern(docId)) {
            if (!hasQrSet.has(docId)) {
              withoutCount += 1;
            }
          }
        });
